...
public class AIClient implements Runnable
{
   ...
   public int DeepeningSearch(GameState game)
   {
       int bestMove = 0;
       int bestState = -1000000; //Arbritrarily high negative value
       int state;
       
     
       int firstPlayer = player;
       int secondPlayer = player;
       ...
       for(int i=1; i < 7 ; i++)
       {
           ...
           while (true) {
               ...
               if (game.moveIsPossible(i))
               {
                   ...
                   //state=minMax(gamestate, depth,!firstPlayer, -1000000, 1000000);
                   
                   state=minMax(gamestate, depth,secondPlayer, -1000000, 1000000);
   ...
   public int minMax(GameState minBoard, int depth,int player, int alpha, int beta)
   {
       ...
       for(int i = 1;i<7;i++)
       {
           ...
           if(player == secondPlayer) {
               bestState = 1000000;
               state=minMax(gamestate,--depth,secondPlayer, alpha, beta);
               ...
               bestState = -1000000;
              state=minMax(gamestate,--depth,firstPlayer, alpha, beta);
